...
GUIDANCE.update(
    {
        "open-review": (
            {"target_kind": "plan", "target_id": "plan_<returned-id>", "scope": "portable"},
            "An opaque review_id that explicitly permits one retained target for a portable host.",
            "Opening records disclosure scope only; it does not start model work or rendering.",
            "Use the returned review ID and configure the adapter with it. A target must already be retained.",
        ),
        "review-snapshot": (
            {"review_id": "review_<returned-id>"},
            "A bounded review snapshot with workspace/finding and raw draft when retained.",
            "Reads shared state without applying edits, running a model or rendering.",
            "REVIEW_MISSING requires the original state folder and explicit review authorization.",
        ),
        "get-review-draft": (
            {"review_id": "review_<returned-id>", "base_plan_id": "plan_<returned-id>"},
            "One full retained raw draft, or null if none is retained for this base.",
            "Snapshots expose only the displayed draft and summary metadata for up to eight others.",
            "Use this explicit read after navigation; it never applies edits or starts work.",
        ),
        "get-review-finding-page": (
            {
                "review_id": "review_<returned-id>",
                "finding_id": "finding_<returned-id>",
                "offset": 0,
            ...
            "A linked finding's bounded candidate page and total.",
            "Large findings are not silently embedded in review snapshots; use this read to paginate.",
            "TARGET_SCOPE_DENIED means the finding was not original or produced by this review.",
        ),
        "navigate-review": (
            {
                "review_id": "review_<returned-id>",
                "view": {"plan_id": "plan_<returned-id>", "playhead": 1.2},
                "expected_version": 1,
            },
            "The next versioned displayed review focus, without a workspace mutation.",
            "Navigation never follows the newest workspace head automatically and never starts work.",
            "REVIEW_CONFLICT requires a new review-snapshot before choosing a target.",
        ),
        "save-review-draft": (
            {
                "review_id": "review_<returned-id>",
                "base_plan_id": "plan_<returned-id>",
                "draft": {},
                "expected_version": 0,
            },
            "The next raw draft version, without a new Plan revision.",
            "Raw incomplete editor values remain separate from immutable applied plans and exports.",
            "DRAFT_CONFLICT means another writer won; read the snapshot and merge explicitly.",
        ),
        "apply-review-changes": (
            {
                "review_id": "review_<returned-id>",
                "plan_id": "plan_<returned-id>",
                "changes": {"title": "Moment"},
            },
            "The exact new immutable Plan revision in the review workspace.",
            "Uses the same optimistic workspace validation as save-workspace.",
            "WORKSPACE_CONFLICT requires loading the current workspace; never retarget a stale plan.",
        ),
        "select-review-candidate": (
            {
                "review_id": "review_<returned-id>",
                "finding_id": "finding_<returned-id>",
                "candidate_id": "candidate_<returned-id>",
            },
            "The same review now focused on a selected plan and its workspace.",
            "Selection is deterministic and does not run another model. It changes review focus but preserves its explicit host authority.",
            "CANDIDATE_MISSING or INVALID_TARGET requires the retained finding/candidate identities.",
        ),
        "admit-operation": (
            {
                "request_id": "render-001",
                "operation": "render",
                "arguments": {"plan_id": "plan_<returned-id>"},
            },
            "An operation admission record, not an artifact receipt.",
            "Persists immutable arguments and effective limits, then uses an owned detached worker. Exact retry reuses admission.",
            "Use get-operation/recover-operations after lost responses. Changed inputs under one ID conflict.",
        ),
        "get-operation": (
            {"operation_id": "operation_<returned-id>"},
            "Retained admission status and terminal domain outcome, if known.",
            "A running status is not artifact success. Reads never replay work.",
            "Use recover-operations after process interruption; uncertain work is never automatically replayed.",
        ),
        "get-operation-result-page": (
            {"operation_id": "operation_<returned-id>", "offset": 0, "limit": 65536},
            "Base64 result bytes plus total byte size.",
            "Large operation results are explicitly limited in summaries; this retrieves one 64 KiB page.",
            "Use returned total_bytes to page. Operation scope remains enforced when review_id is supplied.",
        ),
        "cancel-operation": (
            {"operation_id": "operation_<returned-id>"},
            "Cancellation acknowledgement and current status.",
            "Cancellation is serialized with publication: an already published receipt remains ready.",
            "Poll get-operation for cleanup; acknowledgement alone is not a terminal result.",
        ),
        "recover-operations": (
            {},
            "Known reconciled publications and interrupted operations; replayed is always false.",
            "Checks preassigned output identities before classifying a stale worker.",
            "Unknown model work remains interrupted/uncertain for an explicit new request.",
        ),
        "describe-media": (
            {
                "review_id": "review_<returned-id>",
                "kind": "artifact",
                "identity": "export_<returned-id>",
            },
            "Opaque media ID, MIME, byte length, SHA-256 and maximum chunk size.",
            "Only review-member artifact or observed frame bytes are exposed; no source paths are returned.",
            "MEDIA_SCOPE_DENIED requires an explicit review for the target.",
        ),
        "read-media": (
            {
                "review_id": "review_<returned-id>",
                "media_id": "media_<returned-id>",
                "offset": 0,
                "length": 196608,
            },
            "A bounded byte chunk with integrity metadata.",
            "Checks membership, roots, symlinks and unchanged bytes on every read.",
            "MATERIAL_CHANGED or INVALID_RANGE requires a new safe descriptor or offset.",
        ),
        "get-scoped-saved-outputs": (
            {"offset": 0, "limit": 20, "sort_by": "newest"},
            "A bounded page of authorized export receipts, without local paths or previews.",
            "Use from a caller that supplies explicit review scope or collection authority. It is a retained read and never renders, previews, finds or calls a provider.",
            "OUTPUT_SCOPE_DENIED means the requested export is outside the explicit disclosure scope. Use offset >= 0, limit 1..50 and a supported sort.",
        ),
        "open-scoped-saved-output": (
            {"artifact_id": "export_<returned-id>", "request_id": "open-output-001"},
            "The canonical export review/workspace identity for an authorized output.",
            "Attaches existing retained work only. Its own raw draft is reused and no plan is regenerated or copied into another workspace.",
            "Use an export returned by get-scoped-saved-outputs. Exact request-ID retries recover the same review; changed input conflicts.",
        ),
        "rename-scoped-saved-output": (
            {
                "review_id": "review_<returned-id>",
                "artifact_id": "export_<returned-id>",
                "title": "A better name",
                "expected_plan_id": "plan_<displayed-id>",
            },
            "The updated authorized receipt, retaining the same media hash and artifact identity.",
            "Renames only one authorized export without source access, rendering or applying raw workspace edits.",
            "OUTPUT_SCOPE_DENIED means the review/export pair is not authorized. Exact request-ID retries preserve the original mutation outcome.",
        ),
        "delete-scoped-saved-output": (
            {
                "review_id": "review_<returned-id>",
                "artifact_id": "export_<returned-id>",
                "expected_plan_id": "plan_<displayed-id>",
            },
            "Deletion status for one authorized generated export.",
            "Deliberately removes generated media and its receipt only; source media, plans and other outputs remain retained.",
            "OUTPUT_SCOPE_DENIED requires authorized scope. Inspect DELETE_FAILED; exact request-ID retries do not repeat deletion.",
        ),
        "describe-scoped-saved-output-media": (
            {"artifact_id": "export_<returned-id>"},
            "Opaque MIME, byte size, SHA-256 and a scoped resource URI for one authorized export.",
            "Returns no local path. Readers recheck output membership, symlinks and bytes before disclosure.",
            "Use only an authorized export identity. MATERIAL_CHANGED requires a fresh descriptor.",
        ),
        "read-scoped-saved-output-media": (
            {"artifact_id": "export_<returned-id>", "offset": 0, "length": 196608},
            "One verified bounded byte chunk from an authorized saved export.",
            "Reads through explicit review/collection authority and never accepts a path or source handle.",
            "OUTPUT_SCOPE_DENIED, MATERIAL_CHANGED and INVALID_RANGE require a valid authorized descriptor/offset.",
        ),
    }
)
...
PARAMETERS = {
    "review_id": "opaque retained review identity",
    "target_kind": "string; plan, artifact or finding",
    "target_id": "retained identity selected for review",
    "view": "ReviewView object: explicit plan/finding/candidate/cue/playhead focus",
    "draft": "object; raw editor values, possibly incomplete",
    "expected_version": "integer; current raw draft version",
    "base_plan_id": "plan identity or null for a non-plan draft",
    "expected_plan_id": "exact displayed saved-output receipt plan identity for CAS mutation",
    "request_id": "stable exact retained operation identity",
    "operation": "string; preview, render, find or make",
    "arguments": "object; exact public operation arguments",
    "allow_models": "boolean; direct-library worker permission (portable servers set this at startup)",
    "operation_id": "retained operation admission identity",
    "kind": "string; artifact or frame",
    "identity": "artifact or evidence identity within the retained review",
    "media_id": "opaque scoped material descriptor identity",
    "length": "integer; media chunk size up to 1048576",
    ...
    "offset": "number; caption seconds or nonnegative retained-media/page byte offset, by capability",
    ...
    "scope": "string/null catalog folder or explicit retained-work disclosure authority, by capability",
    "limit": "integer; maximum returned sources/hits or 1..50 saved outputs, by capability",
